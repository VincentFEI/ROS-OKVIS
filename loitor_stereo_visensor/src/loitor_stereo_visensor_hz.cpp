...
void* imu_data_stream(void *)
{
	...
	//std::ofstream imufile("/home/ubuntu/vioproject/src/loitor_stereo_visensor/imu.txt",std::ios::app);//ios::app表示在原文件末尾追加
	//if(!imufile) pthread_exit(NULL);;
	...
	while(!visensor_Close_IMU_viewer)
	{
		if(visensor_imu_have_fresh_data())
			...
			imu_msg.orientation.z=visensor_imudata_pack.qz;
	    		...
	    		//imufile<<imu_msg.linear_acceleration.x<<" "<< imu_msg.linear_acceleration.y<<" "<<imu_msg.linear_acceleration.z<<" ";
	    		//imufile<<imu_msg.angular_velocity.x <<" "<<imu_msg.angular_velocity.y<<" "<<imu_msg.angular_velocity.z<<std::endl;
