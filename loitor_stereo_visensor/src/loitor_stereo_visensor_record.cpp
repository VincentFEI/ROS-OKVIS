...
void* imu_data_stream(void *)
{
	...
	string path = "/home/ubuntu/bagdata";
	string newpath = path + "/imu0.csv";  
	std::ofstream outfile;
	outfile.open(newpath,std::ios::app);  
	outfile<<"timestamp,omega_x,omega_y,omega_z,alpha_x,alpha_y,alpha_z"<<endl;  
	...
	while(!visensor_Close_IMU_viewer)
	{
		if(visensor_imu_have_fresh_data())
			...
			uint64_t time = (uint64_t)imu_time.sec*1000000000ull + (uint64_t)imu_time.nsec;
			string data = std::to_string(time)+","
                                      +std::to_string(imu_msg.angular_velocity.x)+","
                                      +std::to_string(imu_msg.angular_velocity.y)+","
			              +std::to_string(imu_msg.angular_velocity.z)+","
                                      +std::to_string(imu_msg.linear_acceleration.x)+","
			              +std::to_string(imu_msg.linear_acceleration.y)+","
                                      +std::to_string(imu_msg.linear_acceleration.z);
...
{ 
	...
	if(argv[2])		
	{
		if(argv[2]==s)
		{
			std::cout<<"Please input hz number!"<<std::endl;
			if(argv[3])
				hz=std::stoi(argv[3]);
			else
			{
	...
	while (ros::ok())
	{
		...
		else if(visensor_cam_selection==2)
		{
			...
			if(static_ct>=hz_num)
			{
				pub.publish(msg);
				std::string newpath;
				uint64_t time = (uint64_t)msg_time.sec*1000000000ull + (uint64_t)msg_time.nsec;
				newpath = path + "/cam0/" + std::to_string(time)+".png";
				cout<<"left_time : "<<newpath<<endl;
				cout<<"left_time : "<<msg_time.sec<<endl;
				cout<<"left_time : "<<msg_time.nsec<<endl;
				cv::imwrite(newpath,img_left);
				static_ct=0;
			}
		}
