...
unsigned char HFLink::packageAnalysis(void)
{
    ...
    case READ_IMU_DATA :
        ...
        analysis_state=readCommandAnalysis(command_state_ , (unsigned char *)&my_robot->measure_imu_original_data , sizeof(my_robot->measure_imu_original_data));
        break;
...
unsigned char HFLink::masterSendCommand(const Command command_state)
{
    ...
    case SET_ROBOT_SPEED :
        receive_package_renew[(unsigned char)command_state] = 0 ;
        //std::cerr<<"expect_robot_speed.x:  "<<std::fixed<<my_robot->expect_robot_speed.x<<std::endl;
        //std::cerr<<"expect_robot_speed.y:  "<<std::fixed<<my_robot->expect_robot_speed.y<<std::endl;
        //std::cerr<<"expect_robot_speed.z:  "<<std::fixed<<my_robot->expect_robot_speed.z<<std::endl;
        //printf("expect_robot_speed.x:%f\n",&my_robot->expect_robot_speed.x);
        //printf("expect_robot_speed.y:%f\n",&my_robot->expect_robot_speed.y);
        //printf("expect_robot_speed.z:%f\n",&my_robot->expect_robot_speed.z);
