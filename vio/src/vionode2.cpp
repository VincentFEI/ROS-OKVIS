...
public:
  ROSVIOPUB()
  {
    ...
    pointpub = nh.advertise<geometry_msgs::PointStamped>("estimate_pose",100);
  }
  ~ROSVIOPUB(){}
				      ...
				      Eigen::aligned_allocator<okvis::kinematics::Transformation>> &T_SCs)
  {
    ...
    pointstamp.header.stamp.fromNSec(t.toNSec());
    pointstamp.header.frame_id = "map";
    pointstamp.point.x =  T.coeffs()[0];
    pointstamp.point.y =  T.coeffs()[1];
    pointstamp.point.z =  T.coeffs()[2];
    pointpub.publish(pointstamp);
  }
